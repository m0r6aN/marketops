...
{
    [Fact]
    public async Task EvaluateAsync_ActorMismatch_DeniesBeforeKeon()
    {
        var packet = MakePacket(actorId: "not-operator-marketops");
        ...
        Assert.Equal("ACTOR_MISMATCH", result.DenialCode);
    ...
    private static PublishPacket MakePacket(string? actorId = null)
    {
        return new PublishPacket(
            ...
            ActorId: actorId ?? "operator-marketops",
