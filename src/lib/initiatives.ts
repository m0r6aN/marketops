...
export const getInitiativeBySlug = (slug: string) =>
  initiatives.find((initiative) => initiative.slug === slug);
...
// Stub value for later task wiring; kept separate from computed portfolio metrics.
...
export const portfolioMetrics = {
  ...
  activeInitiatives: initiatives.filter((initiative) => initiative.isActive).length,
  needsPositioningReviewCount: initiatives.filter(
    (initiative) => initiative.needsPositioningReview
  ).length,
  launchStageDistribution: initiatives.reduce<Record<InitiativeStageKey, number>>(
    (acc, initiative) => {
